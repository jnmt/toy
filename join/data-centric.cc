...
public:
  void produce() {
    while (cursor < size) {
      parent->consume(table[cursor++]);
    }
  }

public:
  void consume(RECORD *rec) {
    printf("ERROR: ScanOp.consume() should never be called.\n");
  }
};
  ...
  void consume(RECORD *rec) {
    if (isLeft) {
      ...
      if (leftRec->attr[leftAttr] == rightRec->attr[rightAttr])
        parent->consume(join_record(leftRec, rightRec));
    }
  }
};
...
int main(int argc, char *argv[]) {
  ...
  if (n > MAX_TABLE_NUM || n < 2) {
    ...
    begin = cur_time();
    print->open();
    print->produce(); // Execute join in push-fashion
    print->close();
    end = cur_time();
