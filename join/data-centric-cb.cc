...
public:
  virtual void open() {}
  virtual void close() {}
  virtual void exec(std::function<void(RECORD*)>) {}
  virtual void debug() {}
};
...
public:
  void exec(std::function<void(RECORD*)> callback) {
    while (cursor < size) {
...
public:
  void exec(std::function<void(RECORD*)> callback) {
    ...
    left->exec([&](RECORD* rec){
      leftRec = rec;
      right->exec([&](RECORD* rec){
        rightRec = rec;
...
public:
  void exec(void (*callback)(RECORD*)) {
    //printf("DEBUG: %s, %s\n", typeid(this).name(), __func__);
    //child->debug();
    child->exec([&](RECORD* rec){
